...
def _gen_image(brand_kit: dict[str, Any], brief: dict[str, Any]) -> bytes:
    ...
    prompt = (
        ...
        f"• Hero subject takes 35–45% of the frame and SITS IN THE RIGHT-MIDDLE of the canvas\n"
        f"• Subject is centred VERTICALLY in the frame, positioned in the RIGHT HALF horizontally\n"
        ...
        f"• CRITICAL COMPOSITION RULE — the LEFT 50% of the canvas is COMPLETELY EMPTY "
        f"solid {bg_colour} background: no subject, no shadow, no decoration, nothing. "
        f"This entire left half is reserved for headline + CTA text overlay.\n"
        f"• Position the subject ENTIRELY in the right half so it never crosses the vertical centerline. "
        f"Imagine a vertical line at 50% width — nothing crosses to the left of it.\n"
        f"• Subject vertical centre should align with the canvas vertical centre.\n"
        f"• Leave a small clean margin at the top-right and top-left corners for logo overlays.\n"
        f"{partner_note}\n"
