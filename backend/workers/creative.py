...
def _gen_image(brand_kit: dict[str, Any], brief: dict[str, Any]) -> bytes:
    ...
    prompt = (
        ...
        + f"• The text overlay area must be a CLEAN SINGLE-COLOUR region — do NOT add torn-paper "
          f"edges, ripped-edge dividers, textured borders, decorative dividers, or any pattern "
          f"where the image transitions into the text zone. The transition must be either invisible "
          f"or a hard clean edge — not a torn/ripped/deckled paper effect.\n"
        + f"{partner_note}\n"
