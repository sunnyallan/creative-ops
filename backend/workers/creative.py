...
def _gen_image(brand_kit: dict[str, Any], brief: dict[str, Any]) -> bytes:
    ...
    prompt = (
        ...
        f"• Hero subject takes 45–60% of the frame and SITS IN THE UPPER HALF ONLY\n"
        ...
        f"• CRITICAL COMPOSITION RULE — the BOTTOM 45% of the canvas is COMPLETELY EMPTY "
        f"solid {bg_colour} background: no subject, no shadow, no decoration, no food, no objects, "
        f"no plates, no boxes, no garnish — nothing. This whole strip is reserved for headline + CTA text.\n"
        f"• Position the subject ENTIRELY in the upper half of the frame so it never touches "
        f"the lower 45% region. Imagine a horizontal line at 55% height — nothing crosses below it.\n"
        f"{partner_note}\n"
